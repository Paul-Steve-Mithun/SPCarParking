...
export function ExpensesDashboard() {
    ...
    useEffect(() => {
        fetchTransactions();
    }, [selectedMonth, selectedYear, debouncedSearchQuery]);
    ...
    useEffect(() => {
        const filtered = expenses.filter(expense => {
            ...
            return (
                ...
                (expense.spentBy || '').toLowerCase().includes(query) ||
    ...
    const fetchStats = async () => {
        ...
        try {
            ...
            // Optional: toast error for stats failure?
            // kept silent to avoid spamming if both fail
        } finally {
    ...
    // Kept for backward compatibility if needed, but fetchStats/Transactions replace it.
                                                    ...
                                                    {isExpensesLoading ? (
                                                        ...
                                                        filteredData.map((expense, index) => (
                                                            <tr key={expense._id} className={`${isDarkMode ? 'hover:bg-gray-800' : 'hover:bg-gray-50'} cursor-pointer`} onClick={e => {
                                                                ...
                                                                <td className={`px-3 sm:px-6 py-4 whitespace-nowrap text-sm ${isDarkMode ? 'text-yellow-300' : 'text-gray-600'}`}>
                                                                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${expense.transactionMode === 'UPI'
                                                                ...
                                                                <td className={`px-3 sm:px-6 py-4 whitespace-nowrap text-sm ${isDarkMode ? 'text-yellow-300' : 'text-gray-600'}`}>
                                                                    <div className="flex items-center justify-center">
                                                                        {expense.spentBy && expense.spentBy.trim().toLowerCase() === 'mani' ? (
                                                                        ...
                                                                        ) : expense.spentBy && expense.spentBy.trim().toLowerCase() === 'balu' ? (
                                                                            <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium border transition-colors duration-300 ${
                                                                                isDarkMode 
